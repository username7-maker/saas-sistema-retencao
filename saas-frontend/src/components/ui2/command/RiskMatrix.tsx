...
export function RiskMatrix({ segments, className }: RiskMatrixProps) {
  return (
    <div className={cn("divide-y divide-lovable-border/50", className)}>
      {segments.map((segment) => {
        const clampedRate = Math.max(0, Math.min(100, segment.rate ?? 0));
        return (
          <div key={segment.id} className="grid gap-3 py-3 first:pt-0 last:pb-0 md:grid-cols-[auto_1fr_auto] md:items-center">
