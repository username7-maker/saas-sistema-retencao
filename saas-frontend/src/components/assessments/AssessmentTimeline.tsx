  ...
  if (assessments.length === 0) {
    return (
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-panel">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-600">Timeline de avaliações</h3>
        <p className="mt-2 text-sm text-slate-500">Nenhuma avaliação cadastrada.</p>
      </section>
    );
  }
        ...
        {assessments.map((assessment) => (
          <li key={assessment.id} className="rounded-xl border border-slate-200 p-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-sm font-semibold text-slate-700">Avaliação #{assessment.assessment_number}</p>
