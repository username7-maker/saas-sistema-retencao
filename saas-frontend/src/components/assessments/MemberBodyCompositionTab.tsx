...
function deltaTone(deltaValue: number | null, inverse = false): DeltaTone {
  if (deltaValue === null || deltaValue === 0) return "flat";
  if (inverse) return deltaValue < 0 ? "up" : "down";
  return deltaValue > 0 ? "up" : "down";
}

function DeltaIndicator({ value, inverse = false, suffix = "" }: { value: number | null; inverse?: boolean; suffix?: string }) {
  const tone = deltaTone(value, inverse);
  if (value === null) {
    return <span className="text-xs text-lovable-ink-muted">-</span>;
  }
  const abs = Math.abs(value);
  const formatted = `${value > 0 ? "+" : ""}${abs.toFixed(2)}${suffix}`;
  if (tone === "up") {
    return (
      <span className="inline-flex items-center gap-1 text-xs font-semibold text-lovable-success">
        <ArrowUp size={12} />
        {formatted}
      </span>
    );
  ...
  if (tone === "down") {
    return (
      <span className="inline-flex items-center gap-1 text-xs font-semibold text-lovable-danger">
        <ArrowDown size={12} />
        {formatted}
      </span>
    );
  ...
  return (
    <span className="inline-flex items-center gap-1 text-xs font-semibold text-lovable-ink-muted">
      <Minus size={12} />
      {formatted}
    </span>
  );
...
export function MemberBodyCompositionTab({ memberId }: Props) {
  ...
  const weightDelta = delta(toNum(latest?.weight_kg), toNum(previous?.weight_kg));
  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent>
          {!latest ? (
            <p className="text-sm text-lovable-ink-muted">Sem avaliacao de bioimpedancia registrada ainda.</p>
          ) : (
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              <MetricCard label="Peso total" value={fmt(latest.weight_kg, " kg")} deltaNode={<DeltaIndicator value={weightDelta} inverse />} />
              <MetricCard label="% Gordura" value={fmt(latest.body_fat_percent, "%")} deltaNode={<DeltaIndicator value={bodyFatDelta} inverse suffix="%" />} />
              <MetricCard label="Massa muscular" value={fmt(latest.muscle_mass_kg, " kg")} deltaNode={<DeltaIndicator value={muscleDelta} />} />
              <MetricCard label="Gordura visceral" value={`Nivel ${fmt(latest.visceral_fat_level)}`} deltaNode={<DeltaIndicator value={visceralDelta} inverse />} />
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <CardContent>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <section className="rounded-xl border border-lovable-border bg-lovable-surface-soft p-3">
                <p className="text-xs font-semibold uppercase tracking-wider text-lovable-ink-muted">Leitura por foto (MVP)</p>
        ...
        <div className="flex flex-col gap-4">
          <h3 className="text-sm font-semibold uppercase tracking-wider text-lovable-ink-muted">Historico de Avaliacoes</h3>
            ...
            evaluations?.map((ev) => (
              <Card key={ev.id}>
                <CardContent className="pt-4">
                  ...
                  {ev.notes && <p className="mt-3 rounded-lg bg-lovable-surface-soft px-3 py-2 text-xs text-lovable-ink-muted">{ev.notes}</p>}
                </CardContent>
              </Card>
            ))}
        </div>
...
function MetricCard({ label, value, deltaNode }: { label: string; value: string; deltaNode: ReactNode }) {
  return (
    <article className="rounded-xl border border-lovable-border bg-lovable-surface-soft p-3">
      ...
      <div className="mt-1">{deltaNode}</div>
    </article>
  );
}
