...
export function NotificationsPage() {
  ...
  return (
    <section className="space-y-6">
      <header>
        <h2 className="font-heading text-3xl font-bold text-slate-900">Notificacoes In-App</h2>
        <p className="text-sm text-slate-500">Alertas internos de retencao e operacao. Nao ha envio automatico de WhatsApp.</p>
      </header>
            ...
            <tbody>
              {notificationsQuery.data.items.map((item) => (
                <tr key={item.id} className="border-t border-slate-100">
                  <td className="px-2 py-2 uppercase text-xs text-slate-600">{item.category}</td>
                  <td className="px-2 py-2 font-medium text-slate-800">{item.title}</td>
                  <td className="px-2 py-2 text-slate-600">{item.message}</td>
                  <td className="px-2 py-2 text-slate-500">{new Date(item.created_at).toLocaleString()}</td>
