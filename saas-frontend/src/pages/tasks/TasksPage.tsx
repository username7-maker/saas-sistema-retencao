...
export function TasksPage() {
  ...
  return (
    <section className="space-y-6">
      <header>
        <h2 className="font-heading text-3xl font-bold text-lovable-ink">Tasks Internas</h2>
        <p className="text-sm text-lovable-ink-muted">Kanban simplificado para atendimento e retencao.</p>
      </header>

      <div className="grid gap-4 md:grid-cols-3">
        {statusSequence.map((status) => (
          <section key={status} className="rounded-2xl border border-lovable-border bg-lovable-surface p-4 shadow-panel">
            <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-lovable-ink-muted">{status}</h3>
            <div className="space-y-3">
              {tasksQuery.data.items
                .filter((task) => task.status === status)
                .map((task) => (
                  <article key={task.id} className="rounded-xl border border-lovable-border bg-lovable-surface-soft p-3">
                    <p className="text-sm font-semibold text-slate-800">{task.title}</p>
                    <p className="mt-1 text-xs text-lovable-ink-muted">Prioridade: {task.priority}</p>
