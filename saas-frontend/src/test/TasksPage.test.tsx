...
async function openCompleteList() {
  fireEvent.click(await screen.findByRole("tab", { name: "Lista completa" }));
...
describe("TasksPage", () => {
  ...
  it("renders triage as the default operational view with fixed sections", async () => {
    ...
    expect(screen.getByRole("tab", { name: "Modo execucao" })).toBeInTheDocument();
    expect(screen.getByRole("tab", { name: "Lista completa" })).toBeInTheDocument();
    expect(screen.getAllByRole("tab", { name: "Onboarding" }).length).toBeGreaterThan(0);
  ...
  it("keeps onboarding in a dedicated tab with journey buckets and the queue CTA", async () => {
    ...
    fireEvent.click(screen.getAllByRole("tab", { name: "Onboarding" })[0]);
