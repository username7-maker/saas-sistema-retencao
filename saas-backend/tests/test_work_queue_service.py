...
def test_wq_reuse_ai_triage_execute_awaiting_outcome_validates_prepared_task_before_returning_id(monkeypatch):
    ...
    recommendation = AITriageRecommendation(
        ...
        outcome_state="pending",
        last_refreshed_at=datetime(2026, 7, 13, 12, 0, tzinfo=timezone.utc),
        payload_snapshot={
            "subject_name": "Aluno",
            "priority_bucket": "medium",
            "why_now_summary": "Ja preparado",
            "why_now_details": [],
            "recommended_action": "Criar tarefa",
            "recommended_channel": "task",
            "recommended_owner": {"user_id": None, "role": "reception", "label": "Recepcao"},
            "suggested_message": "Revisar onboarding",
            "expected_impact": "Evitar dropout",
            "metadata": {"prepared_task_id": str(raw_task_id), "onboarding_score": 50},
        },
    )
    ...
    assert result.item.canonical_task_id is None
