...
def test_slaughter_black_child_uses_schofield_and_requires_measured_muscle_mass():
    preview = preview_anthropometric_assessment(slaughter_payload())

    assert float(preview["results"]["body_fat_pct"]) == pytest.approx(19.55)
    assert float(preview["results"]["basal_metabolic_rate"]) == pytest.approx(1723.74)
    assert preview["basal_metabolic_rate_origin"] == "schofield_hw_1985"
    assert preview["results"]["muscle_mass_kg"] is None
    assert preview["muscle_mass_origin"] == "unavailable"
    assert "muscle_measurement_required" in preview["snapshot"]["flags"]
    assert "poortmans_population_not_validated" in preview["snapshot"]["flags"]
    ...
    assert preview["snapshot"]["muscle_mass_calculation"]["origin"] == "unavailable"


def test_white_child_uses_poortmans_with_corrected_circumferences():
    preview = preview_anthropometric_assessment(slaughter_payload(ethnicity="white"))
    ...
    assert preview["muscle_mass_origin"] == "poortmans_2005"
    assert preview["snapshot"]["muscle_mass_calculation"]["measurement_side"] == "right"
    assert "0.0064*CAG^2" in preview["snapshot"]["muscle_mass_calculation"]["formula"]
...
def test_premium_report_contains_lee_muscle_mass_bmr_source_and_warning():
    ...
    assert muscle_metric["label"] == "Massa muscular"
    assert muscle_metric["source_label"] == "Massa muscular estimada por Lee (2000)"
    assert bmr_metric["label"] == "TMB"
    ...
    assessment.muscle_mass_origin = "unavailable"
    ...
    assert "Massa muscular indisponivel" in without_muscle.parameters["methodological_note"]
