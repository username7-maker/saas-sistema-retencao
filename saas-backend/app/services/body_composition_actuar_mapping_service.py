...
def build_body_composition_canonical_payload(member: Member, evaluation: BodyCompositionEvaluation) -> dict[str, Any]:
    ...
    return {
        ...
        "total_energy_kcal": _to_float(getattr(evaluation, "total_energy_kcal", None)),
...
def build_actuar_field_mapping(
    ...
    mappings = [
        ...
        _mapping("target_weight_kg", "target_weight_kg", payload["target_weight_kg"], "non_critical_direct"),
        # The current Actuar form calculates these read-only fields from editable inputs.
        ...
        _mapping("body_water_pct", "body_water_percent", payload["body_water_pct"], "unsupported", supported=False),
        _mapping("bmr_kcal", "bmr_kcal", payload["bmr_kcal"], "unsupported", supported=False),
        _mapping("total_energy_kcal", "total_energy_kcal", payload["total_energy_kcal"], "non_critical_direct"),
        _mapping("visceral_fat", None, payload["visceral_fat"], "unsupported", supported=False),
        _mapping("notes", None, payload["notes"], "unsupported", supported=False),
    ]
