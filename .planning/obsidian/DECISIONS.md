...
## 2026-09-11 - Bioimpedancia Recuperavel e Edicao Concorrente
A bioimpedancia passa a usar `deleted_at`, preservando o registro e a auditoria. Leituras operacionais ignoram registros removidos; jobs de sincronizacao ainda abertos sao cancelados e tarefas concluidas permanecem preservadas. Edicoes exigem `expected_updated_at` e bloqueiam sobrescrita concorrente com `409 Conflict`.

## 2026-09-11 - Recuperacao de Leitura com Divergencia Explicita
A segunda leitura do recibo tambem pode recuperar campos preenchidos sem evidencia valida, nao apenas campos nulos. Ela possui limite proprio de 20 segundos. Quando duas leituras retornam valores diferentes, o primeiro nao e substituido silenciosamente: o campo fica em conflito para conferencia. Papel branco com texto legivel nao deve ser classificado como reflexo apenas pela proporcao de pixels claros.

## 2026-09-10 - Recuperacao Deterministica de Recibos Termicos
A leitura assistida de bioimpedancia usa primeiro a variante normal corrigida e somente faz uma segunda chamada de IA quando idade, sexo, altura, peso ou IMC nao forem encontrados ou nao tiverem evidencia valida. A segunda chamada recebe uma variante deterministica com normalizacao de fundo, contraste local, nitidez moderada e limiarizacao adaptativa. Ela preenche campos ausentes; divergencias com valores da primeira leitura exigem conferencia e nunca causam substituicao silenciosa. Falha da recuperacao preserva o resultado inicial. Filtros generativos ou super-resolucao generativa nao sao permitidos, pois podem inventar digitos clinicos; imagens irrecuperaveis continuam exigindo nova foto.
